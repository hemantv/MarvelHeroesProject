...
const Hero = ({id, color, title, name, description, image, scrollX}) => {
  const backgroundAnim = React.useRef(new Animated.Value(VALUE_BACKGROUND))
    .current;
  const knowMoreAnim = React.useRef(new Animated.Value(VALUE_KNOWMORE)).current;
  const foregroundAnim = React.useRef(new Animated.Value(VALUE_FOREGROUND))
    .current;
  const background2Anim = React.useRef(new Animated.Value(VALUE_BACKGROUND2))
    .current;
